...
class SimpleTask(TaskBase):
    ...
    # @safe_try
    def train(self):
        writer = SummaryWriter(log_dir=self.config.task_dir+"/logs")
        ...
        self.model, self.optimizer, train_loader, self.lr_scheduler = accelerator.prepare(self.model, self.optimizer, train_loader, self.lr_scheduler)
        ...
        for epoch_idx in range(self.history["curr_epoch"], self.config.num_epochs):
            ...
            for batch_idx, batch in enumerate(loader):
                with accelerator.accumulate(self.model):
                    ...
                    if self.config.bce == True: 
                        pass
                        # writer.add_scalar("Train/BCELoss", result["bce_loss"].item(), total_steps)
                        # writer.add_scalar("Train/BCEMeanLoss", result["bce_loss_mean"].item(), total_steps)
                    ...
                    if (
                        ...
                        # if not self.config.fewshot or not self.config.zeroshot:
