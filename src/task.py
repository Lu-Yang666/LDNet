...
class SchemaGuidedInstructBertTask(MrcTaggingTask):
    ...
    def init_model(self):
        if self.config.kd == True and self.config.generate_logits == False: 
            teacher_logits = torch.load(self.config.kd_file)
            teacher_logits = {k : v for d in teacher_logits for k, v in d.items()}
        else:
            teacher_logits = None

        self.model = SchemaGuidedInstructBertModel(
            self.config.plm_dir,
            # tokenizer=self.transform.tokenizer,
            vocab_size=len(self.transform.tokenizer),
            kd=self.config.kd,
            teacher_logits=teacher_logits,
            generate_logits = self.config.generate_logits,
            fewshot=self.config.fewshot,
            ablation=self.config.lddrop,
            droprate=self.config.droprate,
            only_use_mr=self.config.only_use_mr,
        )
