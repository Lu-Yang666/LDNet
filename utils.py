...
from typing import Any, List, Tuple, Union,Iterable, Iterator
import torch
import numpy as np
...
def find_paths_from_adj_mat(adj_mat: torch.Tensor,nsw_connections) -> list[tuple[int]]:
    ...
    def track(path: tuple[int], c: int):
        ...
        while stack:
            ...
            if c in adj_map:
                for n in adj_map[c]:
                    if (path[0],n) in nsw_connections:
                        continue
                    if (c, n) in visited:
                        continue
                    visited.add((c, n))
...
def decode_nnw_nsw_thw_mat(
    ...
    for ins_id in range(ins_num):
        ...
        nnw_paths = find_paths_from_adj_mat(ins_mat[nnw_id, ...],nsw_connections)
        end_start_to_paths = defaultdict(set)
        for path in nnw_paths:
            end_start_to_paths[(path[-1], path[0])].add(path)
        thw_pairs = ins_mat[thw_id, ...].detach().nonzero().tolist()
